...
void Clock_IR_Start(void) 
{
    ...
    Clock_IR_CLKEN |= Clock_IR_CLKEN_MASK;
	Clock_IR_CLKSTBY |= Clock_IR_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_IR_Stop
********************************************************************************
*
* Summary:
...
void Clock_IR_Stop(void) 
{
    ...
    Clock_IR_CLKEN &= (uint8)(~Clock_IR_CLKEN_MASK);
	Clock_IR_CLKSTBY &= (uint8)(~Clock_IR_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_IR_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_IR_CLKEN & Clock_IR_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_IR_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_IR_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_IR__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_IR_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_IR_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_IR_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_IR_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_IR_GetSourceRegister();
    uint16 oldDivider = Clock_IR_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_IR_CLKEN & Clock_IR_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_IR_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_IR_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_IR_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_IR_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_IR_MOD_SRC |= modeBitMask & (uint8)Clock_IR_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_IR_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_IR_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_IR_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_IR_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_IR_MOD_SRC & (uint8)(Clock_IR_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_IR_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_IR_GetDividerRegister();
    uint8 oldSrc = Clock_IR_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_IR_MOD_SRC |= CYCLK_SSS;
        Clock_IR_MOD_SRC =
            (Clock_IR_MOD_SRC & (uint8)(~Clock_IR_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_IR_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_IR_MOD_SRC & Clock_IR_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_IR_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_IR_PHASE = clkPhase & Clock_IR_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_IR_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_IR_PHASE & Clock_IR_PHASE_MASK;
}
