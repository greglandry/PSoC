...
/*******************************************************************************
* Function Name: CyClock_100kHz_Start
********************************************************************************
*
* Summary:
...
void CyClock_100kHz_Start(void) 
{
    ...
    CyClock_100kHz_CLKEN |= CyClock_100kHz_CLKEN_MASK;
	CyClock_100kHz_CLKSTBY |= CyClock_100kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyClock_100kHz_Stop
********************************************************************************
*
* Summary:
...
void CyClock_100kHz_Stop(void) 
{
    ...
    CyClock_100kHz_CLKEN &= (uint8)(~CyClock_100kHz_CLKEN_MASK);
	CyClock_100kHz_CLKSTBY &= (uint8)(~CyClock_100kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyClock_100kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyClock_100kHz_CLKEN & CyClock_100kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyClock_100kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyClock_100kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyClock_100kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyClock_100kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyClock_100kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyClock_100kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyClock_100kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyClock_100kHz_GetSourceRegister();
    uint16 oldDivider = CyClock_100kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyClock_100kHz_CLKEN & CyClock_100kHz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyClock_100kHz_DIV_PTR, clkDivider);
                CyClock_100kHz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyClock_100kHz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyClock_100kHz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyClock_100kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CyClock_100kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyClock_100kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyClock_100kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyClock_100kHz_MOD_SRC |= modeBitMask & (uint8)CyClock_100kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyClock_100kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyClock_100kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyClock_100kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyClock_100kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyClock_100kHz_MOD_SRC & (uint8)(CyClock_100kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyClock_100kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyClock_100kHz_GetDividerRegister();
    uint8 oldSrc = CyClock_100kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyClock_100kHz_MOD_SRC |= CYCLK_SSS;
        CyClock_100kHz_MOD_SRC =
            (CyClock_100kHz_MOD_SRC & (uint8)(~CyClock_100kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyClock_100kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyClock_100kHz_MOD_SRC & CyClock_100kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyClock_100kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyClock_100kHz_PHASE = clkPhase & CyClock_100kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyClock_100kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyClock_100kHz_PHASE & CyClock_100kHz_PHASE_MASK;
}
