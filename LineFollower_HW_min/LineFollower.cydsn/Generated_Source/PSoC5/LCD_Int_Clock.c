...
/*******************************************************************************
* Function Name: LCD_Int_Clock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_Start(void) 
{
    /* Set the bit to enable the clock. */
    LCD_Int_Clock_CLKEN |= LCD_Int_Clock_CLKEN_MASK;
	LCD_Int_Clock_CLKSTBY |= LCD_Int_Clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    LCD_Int_Clock_CLKEN &= (uint8)(~LCD_Int_Clock_CLKEN_MASK);
	LCD_Int_Clock_CLKSTBY &= (uint8)(~LCD_Int_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_StopBlock(void) 
{
    if ((LCD_Int_Clock_CLKEN & LCD_Int_Clock_CLKEN_MASK) != 0u)
    {
...
#if defined(LCD_Int_Clock__CFG3)
        CLK_DIST_AMASK = LCD_Int_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LCD_Int_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LCD_Int_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LCD_Int_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LCD_Int_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        LCD_Int_Clock_CLKSTBY &= (uint8)(~LCD_Int_Clock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = LCD_Int_Clock_GetSourceRegister();
    uint16 oldDivider = LCD_Int_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LCD_Int_Clock_CLKEN & LCD_Int_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LCD_Int_Clock_DIV_PTR, clkDivider);
                LCD_Int_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LCD_Int_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LCD_Int_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LCD_Int_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LCD_Int_Clock_GetDividerRegister(void) 
{
    return CY_GET_REG16(LCD_Int_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_SetModeRegister(uint8 modeBitMask) 
{
    LCD_Int_Clock_MOD_SRC |= modeBitMask & (uint8)LCD_Int_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_ClearModeRegister(uint8 modeBitMask) 
{
    LCD_Int_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LCD_Int_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_Int_Clock_GetModeRegister(void) 
{
    return LCD_Int_Clock_MOD_SRC & (uint8)(LCD_Int_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = LCD_Int_Clock_GetDividerRegister();
    uint8 oldSrc = LCD_Int_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LCD_Int_Clock_MOD_SRC |= CYCLK_SSS;
        LCD_Int_Clock_MOD_SRC =
            (LCD_Int_Clock_MOD_SRC & (uint8)(~LCD_Int_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_Int_Clock_GetSourceRegister(void) 
{
    return LCD_Int_Clock_MOD_SRC & LCD_Int_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LCD_Int_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_Int_Clock_SetPhaseRegister(uint8 clkPhase) 
{
    LCD_Int_Clock_PHASE = clkPhase & LCD_Int_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LCD_Int_Clock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_Int_Clock_GetPhaseRegister(void) 
{
    return LCD_Int_Clock_PHASE & LCD_Int_Clock_PHASE_MASK;
}
