...
*
*******************************************************************************/
void Clock_MOT_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_MOT_CLKEN |= Clock_MOT_CLKEN_MASK;
	Clock_MOT_CLKSTBY |= Clock_MOT_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_MOT_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_MOT_CLKEN &= (uint8)(~Clock_MOT_CLKEN_MASK);
	Clock_MOT_CLKSTBY &= (uint8)(~Clock_MOT_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_MOT_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_StopBlock(void) 
{
    if ((Clock_MOT_CLKEN & Clock_MOT_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_MOT__CFG3)
        CLK_DIST_AMASK = Clock_MOT_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_MOT_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_MOT__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_MOT_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_MOT_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_MOT_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_MOT_CLKSTBY &= (uint8)(~Clock_MOT_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_MOT_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_MOT_GetSourceRegister();
    uint16 oldDivider = Clock_MOT_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_MOT_CLKEN & Clock_MOT_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_MOT_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_MOT_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_MOT_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_MOT_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_MOT_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_SetModeRegister(uint8 modeBitMask) 
{
    Clock_MOT_MOD_SRC |= modeBitMask & (uint8)Clock_MOT_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_MOT_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_MOT_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_MOT_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_MOT_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_MOT_GetModeRegister(void) 
{
    return Clock_MOT_MOD_SRC & (uint8)(Clock_MOT_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_MOT_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_MOT_GetDividerRegister();
    uint8 oldSrc = Clock_MOT_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_MOT_MOD_SRC |= CYCLK_SSS;
        Clock_MOT_MOD_SRC =
            (Clock_MOT_MOD_SRC & (uint8)(~Clock_MOT_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_MOT_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_MOT_GetSourceRegister(void) 
{
    return Clock_MOT_MOD_SRC & Clock_MOT_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_MOT_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_MOT_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_MOT_PHASE = clkPhase & Clock_MOT_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_MOT_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_MOT_GetPhaseRegister(void) 
{
    return Clock_MOT_PHASE & Clock_MOT_PHASE_MASK;
}
